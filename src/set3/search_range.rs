...
fn binary_search_leftmost<T>(nums: &[T], target: &T) -> Option<usize> 
where T: Eq + Ord {
    if nums.is_empty() {
        return None;
    }
...
/// *Space Complexity*: O(1)
/// 
fn binary_search_rightmost<T>(nums: &[T], target: &T) -> Option<usize> 
where T: Eq + Ord {
    if nums.is_empty() {
        return None;
    }
...
where T: Ord + Eq
{
    match binary_search_leftmost(nums, target) {
        ...
        Some(first) => 
            Some((  first,
                    binary_search_rightmost(nums, target).unwrap()) 
                )
    }
}
